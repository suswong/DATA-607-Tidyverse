...
Use `mutate` to create a new variable, `Unemployment_rate`. To find the unemployment rate,
...
We can also use the same code as in question 1. Simply, just replace `Major_category` with `Unemployment_rate` and rename the axes with their appropriate names. 
...
The top three major category that has the highest unemployment rate are Social Science, Arts, and Humanities & Liberal Arts. The top three major category that has the lowesr unemployment rate are Education, Physical Sciences, and Agriculture & Natural Resources. 
